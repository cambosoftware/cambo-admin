<script setup>
import { computed } from 'vue'

const props = defineProps({
    variant: {
        type: String,
        default: 'info',
        validator: (v) => ['info', 'success', 'warning', 'danger'].includes(v)
    },
    title: {
        type: String,
        default: null
    },
    dismissible: {
        type: Boolean,
        default: false
    },
    icon: {
        type: Boolean,
        default: true
    },
    border: {
        type: String,
        default: 'none',
        validator: (v) => ['none', 'left', 'top'].includes(v)
    }
})

const emit = defineEmits(['dismiss'])

const variantClasses = computed(() => {
    const variants = {
        info: 'bg-sky-50 text-sky-800',
        success: 'bg-emerald-50 text-emerald-800',
        warning: 'bg-amber-50 text-amber-800',
        danger: 'bg-red-50 text-red-800'
    }
    return variants[props.variant]
})

const borderClasses = computed(() => {
    if (props.border === 'none') return ''
    const borders = {
        left: { info: 'border-l-4 border-sky-500', success: 'border-l-4 border-emerald-500', warning: 'border-l-4 border-amber-500', danger: 'border-l-4 border-red-500' },
        top: { info: 'border-t-4 border-sky-500', success: 'border-t-4 border-emerald-500', warning: 'border-t-4 border-amber-500', danger: 'border-t-4 border-red-500' }
    }
    return borders[props.border][props.variant]
})

const iconColor = computed(() => {
    const colors = { info: 'text-sky-500', success: 'text-emerald-500', warning: 'text-amber-500', danger: 'text-red-500' }
    return colors[props.variant]
})

const dismissColor = computed(() => {
    const colors = { info: 'text-sky-500 hover:bg-sky-100', success: 'text-emerald-500 hover:bg-emerald-100', warning: 'text-amber-500 hover:bg-amber-100', danger: 'text-red-500 hover:bg-red-100' }
    return colors[props.variant]
})

const iconPaths = {
    info: 'M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a.75.75 0 000 1.5h.253a.25.25 0 01.244.304l-.459 2.066A1.75 1.75 0 0010.747 15H11a.75.75 0 000-1.5h-.253a.25.25 0 01-.244-.304l.459-2.066A1.75 1.75 0 009.253 9H9z',
    success: 'M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z',
    warning: 'M8.485 2.495c.673-1.167 2.357-1.167 3.03 0l6.28 10.875c.673 1.167-.17 2.625-1.516 2.625H3.72c-1.347 0-2.189-1.458-1.515-2.625L8.485 2.495zM10 5a.75.75 0 01.75.75v3.5a.75.75 0 01-1.5 0v-3.5A.75.75 0 0110 5zm0 9a1 1 0 100-2 1 1 0 000 2z',
    danger: 'M10 18a8 8 0 100-16 8 8 0 000 16zM8.28 7.22a.75.75 0 00-1.06 1.06L8.94 10l-1.72 1.72a.75.75 0 101.06 1.06L10 11.06l1.72 1.72a.75.75 0 101.06-1.06L11.06 10l1.72-1.72a.75.75 0 00-1.06-1.06L10 8.94 8.28 7.22z'
}
</script>

<template>
    <div
        :class="['rounded-lg p-4', variantClasses, borderClasses]"
        role="alert"
    >
        <div class="flex">
            <!-- Icon -->
            <div v-if="icon" :class="['flex-shrink-0 mr-3', iconColor]">
                <svg class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                    <path fill-rule="evenodd" :d="iconPaths[variant]" clip-rule="evenodd" />
                </svg>
            </div>

            <!-- Content -->
            <div class="flex-1 min-w-0">
                <h3 v-if="title" class="text-sm font-semibold mb-1">
                    {{ title }}
                </h3>
                <div class="text-sm" :class="title ? 'opacity-90' : ''">
                    <slot />
                </div>
            </div>

            <!-- Dismiss -->
            <button
                v-if="dismissible"
                type="button"
                :class="['ml-3 flex-shrink-0 p-1 rounded-lg transition-colors cursor-pointer', dismissColor]"
                @click="$emit('dismiss')"
            >
                <svg class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
                    <path d="M6.28 5.22a.75.75 0 00-1.06 1.06L8.94 10l-3.72 3.72a.75.75 0 101.06 1.06L10 11.06l3.72 3.72a.75.75 0 101.06-1.06L11.06 10l3.72-3.72a.75.75 0 00-1.06-1.06L10 8.94 6.28 5.22z" />
                </svg>
            </button>
        </div>
    </div>
</template>
